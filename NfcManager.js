...
class Iso15693HandlerIOS {
  ...
  sendRequest({ flags, commandCode, dataBlock }) {
    return callNative('iso15693_sendRequest', [{ flags, commandCode, dataBlock }])
  }
}
    ...
    if (!data || !Array.isArray(data) || data.length !== this.MIFARE_ULTRALIGHT_PAGE_SIZE) {
      return Promise.reject(
        `data should be a non-empty Array[${
          this.MIFARE_ULTRALIGHT_PAGE_SIZE
        }] of integers (0 - 255)`
      )
    }
...
export { ByteParser, NdefParser, NfcTech, NfcEvents, NfcAdapter, Ndef, Nfc15693RequestFlagIOS }
